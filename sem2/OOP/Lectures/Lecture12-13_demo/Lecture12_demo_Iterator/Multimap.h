...
{
public:
	virtual void add(std::string key, const Castle& value) = 0;
...
{
private:
	std::pair<std::string, Castle> element;
	Node* next;

public:
	Node(std::pair<std::string, Castle> elem, Node* n) : element{ elem }, next{ n } {}
	std::string getKey() const { return this->element.first; }
	Castle getValue() const { return this->element.second; }
...
public:
	LinkedListMultimap();
	~LinkedListMultimap();
	void add(std::string key, const Castle& value) override;
...
public:
	TreeNode(std::pair<std::string, Castle> elem, TreeNode* l, TreeNode* r) : element{ elem }, left{ l }, right{ r } {}
	std::string getKey() const { return this->element.first; }
	Castle getValue() const { return this->element.second; }
...
private:
	TreeNode* addRec(TreeNode* p, std::string key, const Castle& value);
